...
import InputClient from "./clientlist/InputClient";
...
const Clients = ({ setAuth }) => {
  ...
  const getProfile = async () => {
    try {
      const res = await fetch("http://localhost:5000/clients/clients", {
        method: "GET",
  ...
  return (
    <div>
      <div className="d-flex mt-5 justify-content-around">
        <h1 className="text-center">Añadir / Modificar clientes </h1>
        <Link to="/dashboard" className="btn btn-primary ml-1">
            Freelancers
        </Link>
      ...
      <InputClient setClientsChange={setClientsChange} />
